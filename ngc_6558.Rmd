...
```{r}
ngc_6558 <- read.csv("1657871670551O-result.csv")
```
...
```{r}
model_1 <- lm(ra ~ dec, data = ngc_6558)
```

b. Regression results from the first model using `summary()`.

An increase of one unit of dec is associated with an additional -0.44265 unit decrease in ra. This relationship is statistically significant at < 0.001.
...
```{r}
model_2 <- lm(pmra ~ pmdec, data = ngc_6558)
pm_error <- lm(pmra_error ~ pmdec_error, data = ngc_6558)
```

b. Regression results from the second and proper motion error model using `summary()`.

`Model 2`: An increase of one unit of pmdec is associated with an additional 0.09842 unit increase in pmra. This relationship is statistically significant at < 0.01.

`pm_error`: An increase of one unit of pmdec error is associated with an additional 1.321345  unit increase in pmra error. This relationship is statistically significant at < 0.001.
...
```{r}
model_3 <- lm(phot_g_mean_mag ~ bp_rp, data = ngc_6558)
phot_error <- lm(phot_g_mean_flux_over_error ~ phot_bp_mean_flux_over_error + phot_rp_mean_flux_over_error, data = ngc_6558)
```

b. Regression results from the third model using `summary()`.

An increase of one unit of bp_rp is associated with an additional -0.9643 unit decrease in phot_g_mean_mag. This relationship is statistically significant at < 0.001.

An increase of one unit of bp/rp error is associated with an additional 3.6395 unit and 0.7953 increase in phot_g_mean_mag error. This relationship is statistically significant at < 0.001.
...
```{r}
ggplot(ngc_6558, aes(mh_gspphot)) +
...
```{r}
ggplot(ngc_6558, aes(radial_velocity)) +
  geom_histogram()
```

#### Polygon search for NGC 6558.

(Switch over to Jupyter Lab.)

```{r}
ps_ngc_6558 <- ngc_6558 
