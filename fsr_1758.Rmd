...
```{r}
fsr_1758 <- read.csv("1657871539232O-result.csv")
```
...
```{r}
model_1 <- lm(ra ~ dec, data = fsr_1758)
```

b. Regression results from the first model using `summary()`.

An increase of one unit of dec is associated with an additional -0.59648 unit decrease in ra. This relationship is statistically significant at < 0.001.
...
```{r}
model_2 <- lm(pmra ~ pmdec, data = fsr_1758)
pm_error <- lm(pmra_error ~ pmdec_error, data = fsr_1758)
```

b. Regression results from the second and proper motion error model using `summary()`.

`Model 2`: An increase of one unit of pmdec is associated with an additional 0.04283 unit increase in pmra. This relationship is statistically significant at < 0.1.

`pm_error`: An increase of one unit of pmdec error is associated with an additional 1.619828 unit increase in pmra error. This relationship is statistically significant at < 0.001.
...
```{r}
model_3 <- lm(phot_g_mean_mag ~ bp_rp, data = fsr_1758)
phot_error <- lm(phot_g_mean_flux_over_error ~ phot_bp_mean_flux_over_error + phot_rp_mean_flux_over_error, data = fsr_1758)
```
...
An increase of one unit of bp/rp error is associated with an additional 2.4873 unit and 1.2577 increase in phot_g_mean_mag error. This relationship is statistically significant at < 0.001.
...
#### Polygon search for FSR 1758.

(Switch over to Jupyter Lab.)

```{r}
ps_fsr_1758 <- fsr_1758 
