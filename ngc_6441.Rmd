...
```{r}
ngc_6441<- read.csv("1657871583903O-result.csv")
```
...
```{r}
model_1 <- lm(ra ~ dec, data = ngc_6441)
```

b. Regression results from the first model using `summary()`.

An increase of one unit of dec is associated with an additional 0.5467 unit decrease in ra. This relationship is statistically significant at < 0.001.
...
```{r}
model_2 <- lm(pmra ~ pmdec, data = ngc_6441)
pm_error <- lm(pmra_error ~ pmdec_error, data = ngc_6441)
```

b. Regression results from the second and proper motion error model using `summary()`.

`Model 2`: An increase of one unit of pmdec is associated with an additional 0.33373 unit increase in pmra. This relationship is statistically significant at < 0.001.

`pm_error`: An increase of one unit of pmdec error is associated with an additional 1.466639 unit increase in pmra error. This relationship is statistically significant at < 0.001.
...
```{r}
model_3 <- lm(phot_g_mean_mag ~ bp_rp, data = ngc_6441)
phot_error <- lm(phot_g_mean_flux_over_error ~ phot_bp_mean_flux_over_error + phot_rp_mean_flux_over_error, data = ngc_6441)
```

b. Regression results from the third model using `summary()`.

An increase of one unit of bp_rp is associated with an additional -1.52680 unit decrease in phot_g_mean_mag. This relationship is statistically significant at < 0.001.

An increase of one unit of bp/rp error is associated with an additional 2.327971 unit and -0.004286 increase and decrease in phot_g_mean_mag error. This relationship is statistically significant at < 0.001 and 1, respectively.
...
```{r}
ggplot(ngc_6441, aes(mh_gspphot)) +
...
```{r}
ggplot(ngc_6441, aes(radial_velocity)) +
  geom_histogram()
```

#### Polygon search for NGC 6441.

(Switch over to Jupyter Lab.)

```{r}
ps_ngc_6441 <- ngc_6441 
select(ps_ngc_6441, source_id, ra, dec, pmra, pmdec, parallax)
```
