...
```{r}
ngc_6453 <- read.csv("1657871624220O-result.csv")
```
...
```{r}
model_1 <- lm(ra ~ dec, data = ngc_6453)
```

b. Regression results from the first model using `summary()`.

An increase of one unit of dec is associated with an additional 0.05855 unit increase in ra. This relationship is statistically significant at < 0.1.
...
```{r}
model_2 <- lm(pmra ~ pmdec, data = ngc_6453)
pm_error <- lm(pmra_error ~ pmdec_error, data = ngc_6453)
```
...
`pm_error`: An increase of one unit of pmdec error is associated with an additional 1.465471 unit increase in pmra error. This relationship is statistically significant at < 0.001.
...
```{r}
model_3 <- lm(phot_g_mean_mag ~ bp_rp, data = ngc_6453)
phot_error <- lm(phot_g_mean_flux_over_error ~ phot_bp_mean_flux_over_error + phot_rp_mean_flux_over_error, data = ngc_6453)
```

b. Regression results from the third model using `summary()`.

An increase of one unit of bp_rp is associated with an additional -1.5059 unit decrease in phot_g_mean_mag. This relationship is statistically significant at < 0.001.

An increase of one unit of bp/rp error is associated with an additional 1.1408 unit and 2.4475 increase in phot_g_mean_mag error. This relationship is statistically significant at < 0.001.
...
```{r}
ggplot(ngc_6453, aes(mh_gspphot)) +
...
```{r}
ggplot(ngc_6453, aes(radial_velocity)) +
  geom_histogram()
```

#### Polygon search for NGC 6453.

(Switch over to Jupyter Lab.)

```{r}
ps_ngc_6453 <- ngc_6453
